...
// MUST be URL strings starting with https://raw.githubusercontent.com/...
const GITHUB_USERS_URL = "https://raw.githubusercontent.com/Porlito/Site/main/data/users.json";
const GITHUB_JOBS_URL = "https://raw.githubusercontent.com/Porlito/Site/main/data/jobs.json";
...
export default function App() {
  ...
  const [newCompany, setNewCompany] = useState({ company: "", logo: "", title: "", description: "" });
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        console.warn("Using fallback demo data. Update GitHub URLs to connect your live files.", err);
  ...
  // Handle Login against Git JSON data
  const handleLogin = (e) => {
    e.preventDefault();
  ...
  // Generate JSON payload for Admin to copy to Git
      ...
      <main className="flex-1 overflow-y-auto p-8">
        ...
        {activeTab === "browse" && (
          <div>
            ...
            {loading ? (
              <p className="text-[#a492c4]">Loading positions from GitHub database...</p>
            ) : (
        ...
        {activeTab === "login" && (
          <div className="max-w-md mx-auto bg-[#25183e] border border-[#3f2a5f] p-6 rounded-xl">
            ...
            <p className="text-sm text-[#a492c4] mb-6">Authenticate against the active GitHub database.</p>
        ...
        {activeTab === "admin" && (
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="bg-[#25183e] border border-[#3f2a5f] p-6 rounded-xl">
              ...
              <p className="text-sm text-[#a492c4] mb-6">Fill out company details to generate formatted JSON for your GitHub database.</p>
              
              <form onSubmit={handleCreateCompany} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs mb-1 text-[#b8a8d9]">Company Name</label>
                    <input required type="text" value={newCompany.company} onChange={(e) => setNewCompany({...newCompany, company: e.target.value})} className="w-full bg-[#120a1f] border border-[#3f2a5f] rounded-lg p-2.5 text-white" />
                  </div>
                  <div>
            ...
            {generatedJson && (
              <div className="bg-[#120a1f] border border-[#3f2a5f] p-4 rounded-xl">
                <h3 className="text-sm font-bold text-[#ff6b6b] mb-2">Copy this text and paste it into your `jobs.json` file on GitHub:</h3>
